...
export default{
    created: function () {
        ...
        self.btnlinks.editbtnlink = "/attendance/empedit/"+self.appid;
        self.btnlinks.duplicatebtnlink = "/attendance/empduplicate/"+self.appid;
        self.btnlinks.printBadgebtnlink = "/attendance/PrintBadge/"+sself.appid;
        $(function(){
            $("#reds").hide();
            //$("#green").hide();
            ...
            $("#export").click(function () {
                self.btnlinks.subordinatebtnlink = "/attendance/hierarchy/"+self.appid;
            });
    ...
    methods: {
        ...
        select: function () {
            var self = this;
            self.$http.post("/recruitment/lastappid2", {"applied_job": self.applied_job}).then(function (res) {
                    var parentdataos = res.body.data[0];
                    self.appid = parentdataos.id;

            self.$http.post("/recruitment/selectemployeeinfo", {"id": self.appid}).then(function (res) {
        ...
        ssubmit: function () {
            var self = this;
            self.$http.post("/recruitment/selectemployeeinfonext", {"id": self.appid}).then(function (res) {
                var parentdata = res.body.data[0];
                self.employeeName = parentdata.employeename;
                self.appid = parentdata.id;
                self.workAddressId = parentdata.work_address_id;
                self.workEmail = parentdata.work_email;
                self.workPhone = parentdata.work_phone;
                self.workMobile = parentdata.work_mobile;
        ...
        psubmit: function () {
            var self = this;
            self.$http.post("/recruitment/selectemployeeinfoback", {"id": self.appid}).then(function (res) {
                var parentdata = res.body.data[0];
                self.employeeName = parentdata.employeename;
                self.appid = parentdata.id;
                self.workAddressId = parentdata.work_address_id;
        ...
        select1: function () {
            ...
            self.$http.post("/recruitment/numemp", {"id": appid}).then(function (res) {
