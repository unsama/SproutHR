...
router.post('/attandanceupdate', function (req, res, next) {
    ...
    connection.query('UPDATE attandance SET CheckIn = "'+req.body.CheckIn+'",CheckOut = "'+req.body.CheckOut+'",emp_id = '+req.body.emp_id+' WHERE emp_id = '+ req.body.id +' AND date = '+ req.body.date +'', function (error, results, fields) {
        if (error) {
            console.log(error.message);
            res.json({"status": "failed", "message": error.message});
        }
...
router.post('/fetchattendancetotal', function (req, res, next) {
    console.log('SELECT *  from attandance where emp_id='+req.body.employee_id+'');
    connection.query('SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(`TotalTime`))) AS total  from attandance where emp_id="'+req.body.employee_id+'"', function (error, results, fields) {
        if (error) res.json({"status": "failed", "message": error.message});
        else{
            console.log(results);
            res.json({"status": "ok", "data": results});
